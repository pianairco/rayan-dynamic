...
/**
 * 
 * Class in which we will keep re-usable executeCommonInsert methods for {@link HibernateTag} and {@link JdbcTag}.
...
public class ProcessingUtil {
	 ...
	 */
// Changed By mohammad For Hibernate Tag
    public String createSortSubquery(final String sortName, final String sortOrder, final String givenAlias, final String defaultsortName) {
        return createSortSubquery(sortName, sortOrder, givenAlias, defaultsortName, true);
    }
// Changed By mohammad For Hibernate Tag

	public String createSortSubquery(final String sortName, final String sortOrder, final String givenAlias, final String defaultsortName, boolean hasAlias) {
		String subquery = null;
// Changed By mohammad
		if ((sortName != null) && (sortOrder != null)) {
      //#1303
      StringTokenizer st = new StringTokenizer(sortName, ",");
      subquery = "";
      while (st.hasMoreTokens()) {
        String sortNamePart = st.nextToken().trim();
        if (sortNamePart.length() > 0) {
          subquery += sortNamePart + " " + (sortOrder.equalsIgnoreCase("1") ? Constants.QL_ASC : Constants.QL_DESC);
          if (st.hasMoreTokens()) {
            subquery += ", ";
          }
        }
      }
      if (hasAlias && !CommonUtils.isNull(givenAlias)) {
        subquery = givenAlias + "." + subquery;
      }
/*
            if (hasAlias && !CommonUtils.isNull(givenAlias))
                subquery = givenAlias + "." + sortName + " " + (sortOrder.equalsIgnoreCase("1")?Constants.QL_ASC:Constants.QL_DESC);
            else
                subquery = sortName + " " + (sortOrder.equalsIgnoreCase("1")?Constants.QL_ASC:Constants.QL_DESC);
*/
    }
		if (subquery == null) {
            if (hasAlias && !CommonUtils.isNull(givenAlias))
...
// Changed By mohammad
	...
	public String createCountQuerySQL(final String table, final String alias, final String whereSubstring) {
		return "SELECT COUNT(*) FROM " + table + " " + alias + " " + whereSubstring.trim();
	}
	...
	public String createCountQueryHQL(final String entity, final String alias) {
		return "select count(" + alias + ") from " + entity + " " + alias;
	}
	...
	public String createCountQueryHQL(final String entity, final String alias, final String whereSubstring) {
		return "select count(" + alias + ") from " + entity + " " + alias + " " + whereSubstring.trim();
	}
	 ...
	 * @param oldParams String with old params generated by {@link RequestDependent#getURIwithOldParameters(HttpServletRequest, String)}
	...
	public String showHtmlWithSearch(final List<HashMap> allSearches, final String oldParams, final String requestUri, final String currSearchField, final String currSearchFrom, final String currSearchTo, final String tableId, final String classSubmit, final String srchBanner) {
        ...
        String htmlFrom = "<input class=\"" + Constants.CSS_SEARCH_INPUT + "\" type=\"text\" name=\"" + searchFrom + "\" MAXLENGTH=\"255\" value=\"" + ((currSearchFrom == null) ? "" : currSearchFrom) + "\"/>";

        // ready HTML code for To: field of search FORM
        String htmlTo = "<input class=\"" + Constants.CSS_SEARCH_INPUT + "\" type=\"text\" name=\"" + searchTo + "\" MAXLENGTH=\"255\" value=\"" + ((currSearchTo == null) ? "" : currSearchTo) + "\"/>";
